 ...
 * <p><b>设计必要性与核心价值：</b></p>
 * <ul>
 *   <li><b>高吞吐数据路径设计（零哈希开销）：</b> 在异步 DAG 流水线实际的运行期数据读写路径中，完全通过原生整型（Primitive Integer）数组索引作为槽位进行数据读写，从而消除了运行时并发读写数据引起的哈希计算、锁争用或内存装箱开销。</li>
 *   <li><b>诊断与静态校验机制：</b> 该符号插槽表提供了从整型插槽索引到人类可读的槽名称的逆向映射。主要在 DAG 静态编译、拓扑启动校验（检测写槽重叠或死锁）、性能度量指标收集（按符号名称记录运行指标）和运行时排错异常诊断（快速打印可读的异常日志）中发挥核心作用。</li>
 *   <li><b>不变性与可预测性：</b> 一旦创建，符号映射和大小完全不可变，具备多线程高并发环境下的强线程安全性。</li>
 * </ul>
 */
